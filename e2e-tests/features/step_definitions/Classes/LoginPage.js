class LoginPage {
    ...
    constructor() {
        ...
        this.header = by.css('#objectives label');
        this.user_data = by.css('li[class=\'btn-group dropdown\']>a');
        ...
        this.avatar_logged = by.css('img[class=\'avatar\']');
    ...
    MouseMove(elem) {
        return browser.actions().mouseMove(element(elem)).perform();
    }

    CheckPrecenceOfTextInElemenValue(elem, value,key) {
        this.WaitUntilElementIsVisible(elem);
        return element(elem).getAttribute(value).then((result) => {
            if (result !== key) {
                throw Error("Element not comprises of - " + key + ". Check text please!");
            }
        });
    }
    ...
    CheckPrecenceOfTextInElement(elem, text) {
        this.WaitUntilElementIsVisible(elem);
        let element_text = element(elem).getText().then((result) => {
            if (result !== text) {
